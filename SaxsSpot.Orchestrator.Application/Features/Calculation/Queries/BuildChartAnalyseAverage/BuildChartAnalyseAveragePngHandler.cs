...
public class BuildChartAnalyseAveragePngHandler(
    INanosystemServiceApiClient nanosystemServiceApiClient,
    IChartService chartService,
    ILogger<BuildChartAnalyseAveragePngHandler> logger
) : IRequestHandler<BuildChartAnalyseAveragePngRequest, IResult<string>>
{
    private static readonly Regex LineFormat = new(@"^\s*(\d+)\s*\(([\d.eE+-]+)\s*-\s*([\d.eE+-]+)\)\s*([\d.eE+-]+)(?:\s+(\d+))?\s*$");

    public async Task<IResult<string>> Handle(
        BuildChartAnalyseAveragePngRequest request,
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(ex, "Failed to build average PNG chart for radial analyses: {Message}", ex.Message);
